...
class FilterComponent extends React.Component {
  ...
  loadAllData() {
    ...
    DashboardService.getSurveysSubmitted(Constants.surveysSubmittedType.all, 0,this.props.model.rightSideModel.sites)
      .then(response => this.props.dispatch(Action.getAction(dashboardActionTypes.SET_SURVEYS_SUBMITTED, response.data.submittedForms)))
      .catch(error => {
    ...
    DashboardService.getRoutesList()
      .then(response => {
        this.props.dispatch(Action.getAction(dashboardActionTypes.SET_ROUTES_DATA, response.data));
  ...
  onBoroughKeywordSearchChange(value) {     
    ...
    //set surveys submitted to all if selected borough is "All" otherwise the selected borough
    let selectedSites = this.props.model.rightSideModel.sites.filter(m=>(m.siteId !=-1 && (value.boroughId == -1 || m.boroughId == value.boroughId)));
    DashboardService.getSurveysSubmitted(Constants.surveysSubmittedType.all, 0,selectedSites)
        .then(response => this.props.dispatch(Action.getAction(dashboardActionTypes.SET_SURVEYS_SUBMITTED, response.data.submittedForms)))
        .catch(error => 
  ...
  onSiteKeywordSearchChange(value) {
    ...
    //set surveys submitted to all if selected borough or site is "All" otherwise the selected borough or site
    if (value.siteId == -1) {
      let boroughID = this.props.model.rightSideModel.filtersModel.selectedBorough.boroughId;
      let selectedSites = this.props.model.rightSideModel.sites.filter(m=>(m.siteId !=-1 && (boroughID == -1 || m.boroughId == boroughID)));
        DashboardService.getSurveysSubmitted(Constants.surveysSubmittedType.all, 0,selectedSites)
          .then(response => this.props.dispatch(Action.getAction(dashboardActionTypes.SET_SURVEYS_SUBMITTED, response.data.submittedForms)))
          .catch(error => 
            this.showErrorMessage(Constants.messages.commonMessages.someErrorOccured)
      ...
      DashboardService.getSurveysSubmitted(Constants.surveysSubmittedType.site, value.siteId)
        .then(response => this.props.dispatch(Action.getAction(dashboardActionTypes.SET_SURVEYS_SUBMITTED, response.data.submittedForms)))
        .catch(error => 
          this.showErrorMessage(Constants.messages.commonMessages.someErrorOccured)
        );
    ...
    if (value.teamId == -1) {
      if (this.props.model.rightSideModel.filtersModel.selectedSite.siteId == -1) {
        if (this.props.model.rightSideModel.filtersModel.selectedBorough.boroughId == -1) {
          ...
          DashboardService.getSurveysSubmitted(Constants.surveysSubmittedType.borough, this.props.model.rightSideModel.filtersModel.selectedBorough.boroughId,this.props.model.rightSideModel.sites)
            .then(response => this.props.dispatch(Action.getAction(dashboardActionTypes.SET_SURVEYS_SUBMITTED, response.data.submittedForms)))
            .catch(error => 
              this.showErrorMessage(Constants.messages.commonMessages.someErrorOccured)
            );
        ...
        DashboardService.getSurveysSubmitted(Constants.surveysSubmittedType.site, this.props.model.rightSideModel.filtersModel.selectedSite.siteId)
          .then(response => this.props.dispatch(Action.getAction(dashboardActionTypes.SET_SURVEYS_SUBMITTED, response.data.submittedForms)))
          .catch(error => 
            this.showErrorMessage(Constants.messages.commonMessages.someErrorOccured)
          );
      ...
      DashboardService.getSurveysSubmitted(Constants.surveysSubmittedType.team, value.teamId)
        .then(response => this.props.dispatch(Action.getAction(dashboardActionTypes.SET_SURVEYS_SUBMITTED, response.data.submittedForms)))
        .catch(error => 
          this.showErrorMessage(Constants.messages.commonMessages.someErrorOccured)
        );
  ...
  fetchFiltersData() {
    this.props.dispatch(Action.getAction(dashboardActionTypes.SET_PANEL_RELOAD_DASHBOARD, true));
    DashboardService.getFiltersData()
      .then(response => {
        this.props.dispatch(Action.getAction(dashboardActionTypes.SET_PANEL_RELOAD_DASHBOARD, false));

        this.props.dispatch(Action.getAction(dashboardActionTypes.SET_FILTERS, response.data));
  ...
  teamFilterOptions(selectedBoroughValue, selectedSiteValue) {
    ...
    } else if (selectedSiteValue.siteId == -1) {
      this.filteredTeams = this.props.model.rightSideModel.teams.filter((team) => (team.teamId == -1));
    } else {
